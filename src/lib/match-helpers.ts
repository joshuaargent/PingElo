...
export async function autoCompleteChallenges(
  ...
  try {
    // For singles: check for challenges between the two players
    // For doubles: check for challenges between any combination of players
    
    if (matchType === 'SINGLES') {
      // Find pending challenges between these two players
      ...
      // For doubles, check for challenges between any team members
      ...
      // Find pending challenges between any players across both teams
      for (const winnerPlayerId of winnerTeam) {
        for (const loserPlayerId of loserTeam) {
          const challenges = await prisma.challenge.findMany({
            where: {
              status: 'ACCEPTED',
              OR: [
                { challengerId: winnerPlayerId, challengedId: loserPlayerId },
                { challengerId: loserPlayerId, challengedId: winnerPlayerId },
              ],
            },
            include: {
              challenger: { select: { id: true, name: true } },
          ...
          for (const challenge of challenges) {
            ...
            await prisma.eloHistory.create({
              data: {
                ...
                eloAfter: 0,
                change: totalPayout,
                description: `Doubles challenge win vs ${winnerPlayerId === challenge.challengerId ? challenge.challenged.name : challenge.challenger.name}`,
                metadata: { challengeId: challenge.id, stakeAmount: challenge.stakeAmount, matchType: 'DOUBLES' },
              },
            });
